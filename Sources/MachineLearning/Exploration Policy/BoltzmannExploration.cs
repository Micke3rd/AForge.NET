...
namespace AForge.MachineLearning
{
    ...
    public class BoltzmannExploration : IExplorationPolicy
    {
        ...
        {
            // actions count
            var actionsCount = actionEstimates.Length;
            // action probabilities
            var actionProbabilities = new double[actionsCount];
            // actions sum
            double sum = 0, probabilitiesSum = 0;

            for (var i = 0; i<actionsCount; i++)
            {
                var actionProbability = Math.Exp(actionEstimates[i]/temperature);
            ...
            if ((double.IsInfinity(probabilitiesSum))||(probabilitiesSum==0))
            {
                ...
                var maxReward = actionEstimates[0];
                var greedyAction = 0;
            ...
            var actionRandomNumber = rand.NextDouble();

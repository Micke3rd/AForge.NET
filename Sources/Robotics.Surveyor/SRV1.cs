	...
	public class SRV1
	{
		...
		public int SendAndReceive(byte[] request,byte[] responseBuffer)
		{
			lock (sync)
			{
				...
				var bytesRead = lastRequestWithReply.BytesRead;
		...
		public Bitmap GetImage()
		{
			...
			var buffer = new byte[768 * 1024];
			// request image
			var bytesRead = SendAndReceive(new byte[] { (byte)'I' },buffer);
			...
			if (bytesRead > 10)
			{
				...
				{
					// extract image size
					var imageSize = System.BitConverter.ToInt32(buffer,6);
		...
		public string GetVersion()
		{
			...
			var read = SendAndReceive(new byte[] { (byte)'V' },response);

			var str = System.Text.ASCIIEncoding.ASCII.GetString(response,0,read);
		...
		public long GetRunningTime()
		{
			...
			var read = SendAndReceive(new byte[] { (byte)'t' },response);
		...
		public float[] UltrasonicPing()
		{
			...
			var read = SendAndReceive(new byte[] { (byte)'p' },response);
			var str = System.Text.ASCIIEncoding.ASCII.GetString(response,0,read);
			...
			try
			{
				...
				for (var i = 0; i < 4; i++)
				{
					distance[i] = int.Parse(strs[i]) / 100f;
				}
		...
		public byte I2CReadByte(byte deviceID,byte register)
		{
			...
			var read = SendAndReceive(new byte[] { (byte)'i',(byte)'r',deviceID,register },response);
			var str = System.Text.ASCIIEncoding.ASCII.GetString(response,0,read);
		...
		public void I2CWriteByte(byte deviceID,byte register,byte byteToWrite)
		{
			...
			var read = SendAndReceive(new byte[] { (byte)'i',(byte)'W',deviceID,register,byteToWrite },response);
		}
		...
		public void I2CWriteWord(byte deviceID,byte register,byte firstByteToWrite,byte secondByteToWrite)
		{
			...
			// use SendAndReceive() to make sure the command was executed successfully
			var read = SendAndReceive(new byte[] { (byte) 'i', (byte) 'W', deviceID, register,
			...
			while (!stopEvent.WaitOne(0,false))
			{
				...
				while (!stopEvent.WaitOne(0,false))
				{
					...
					try
					{
						...
						{
							var bytesToRead = System.Math.Min(readSize,cr.ResponseBuffer.Length);
							...
							{
								// extract image size
								var imageSize = System.BitConverter.ToInt32(cr.ResponseBuffer,6);
								...
								while (!stopEvent.WaitOne(0,false))
								{
									var read = socket.Receive(cr.ResponseBuffer,cr.BytesRead,
		...
		private static void DiscardIncomingData(Socket socket,ManualResetEvent stopEvent)
		{
			...
			while (!stopEvent.WaitOne(0,false))
			{
				var read = socket.Receive(buffer,0,100,SocketFlags.None);

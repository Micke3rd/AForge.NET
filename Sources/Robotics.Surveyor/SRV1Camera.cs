...
namespace AForge.Robotics.Surveyor
{
	...
	public class SRV1Camera: IVideoSource
	{
		...
		public string Source
		{
			get
			{
				return ((communicator != null) && (communicator.EndPoint != null)) ?
		...
		private void WorkerThread()
		{
			...
			while (!stopEvent.WaitOne(0,false))
			{
				try
				{
					...
					var bytesRead = communicator.SendAndReceive(new byte[] { (byte)'I' },buffer);
							...
							(buffer[4] == (byte)'J'))
						{
							...
							if (!stopEvent.WaitOne(0,false))
							{
								try
								{
									...
									var bitmap = (Bitmap)Bitmap.FromStream(new MemoryStream(buffer,10,imageSize));
									framesReceived++;
								...
								if (frameInterval > 0)
								{
									...
									var msec = frameInterval - (int)stopWatch.ElapsedMilliseconds;

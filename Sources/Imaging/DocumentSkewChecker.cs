...
namespace AForge.Imaging
{
	...
	public class DocumentSkewChecker
	{
		...
		public double GetSkewAngle(UnmanagedImage image,Rectangle rect)
		{
			...
			var halfHoughWidth = (int)System.Math.Sqrt(halfWidth * halfWidth + halfHeight * halfHeight);
				...
				for (var y = startY; y < stopY; y++)
				{
					...
					for (var x = startX; x < stopX; x++, src++, srcBelow++)
					{
						...
						if ((*src < 128) && (*srcBelow >= 128))
						{
							...
							{
								var radius = (int)(cosMap[theta] * x - sinMap[theta] * y) + halfHoughWidth;
		...
		{
			// lines count
			var n = System.Math.Min(count,lines.Count);
		...
		private void InitHoughMap()
		{
			if (needToInitialize)
			{
				...
				var minTheta = 90.0 - maxSkewToDetect;

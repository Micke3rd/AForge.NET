...
namespace AForge.Imaging.Filters
{
	...
	public class TransformToPolar : BaseTransformationFilter
	{
		...
		protected override unsafe void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData)
		{
			...
			var diagonalAngle = System.Math.Atan2(cy, cx);

			// offset angle in radians
			var offsetAngleR = offsetAngle / 180 * System.Math.PI;
			if (offsetAngleR < 0)
			{
			...
			for (var y = 0; y < newHeight; y++)
			{
				...
				for (var x = 0; x < newWidth; x++)
				{
					...
					var distance = System.Math.Sqrt(dx * dx + dydy);
					...
					var maxDistance = (oxAngle > diagonalAngle) ? (cy / System.Math.Sin(oxAngle)) : (cx / System.Math.Cos(oxAngle));

...
namespace AForge.Imaging.Filters
{
	...
	public unsafe class PointedMeanFloodFill: BaseInPlacePartialFilter
	{
		...
		protected override unsafe void ProcessFilter(UnmanagedImage image,Rectangle rect)
		{
			...
			{
				var startColor = *CoordsToPointerGray(startingPoint.X,startingPoint.Y);
			...
			{
				var startColor = CoordsToPointerRGB(startingPoint.X,startingPoint.Y);
				...
				var fillR = (byte)meanR;
				var fillG = (byte)meanG;
				var fillB = (byte)meanB;
				...
				var offset = stride - rect.Width * 3;

				// for each line	
				for (var y = startY; y <= stopY; y++)
				{
					// for each pixel
					for (var x = startX; x <= stopX; x++, src += 3)
					{
						if (checkedPixels[y,x])
						{
							src[RGB.R] = fillR;
		...
		private unsafe void LinearFloodFill4Gray(int x,int y)
		{
			...
			ptr = CoordsToPointerGray(leftLineEdge,y);
		...
		private unsafe void LinearFloodFill4RGB(int x,int y)
		{
			...
			// loop to go up and down
			ptr = CoordsToPointerRGB(leftLineEdge,y);

...
namespace AForge.Vision.Motion
{
	...
	public class MotionBorderHighlighting: IMotionProcessing
	{
		...
		public unsafe void ProcessFrame(UnmanagedImage videoFrame,UnmanagedImage motionFrame)
		{
			...
			var srcOffset = videoFrame.Stride - (width - 2) * pixelSize;
			var motionOffset = motionFrame.Stride - (width - 2);
			...
			if (pixelSize == 1)
			{
				...
				for (var y = 1; y < heightM1; y++)
				{
					for (var x = 1; x < widthM1; x++, motion++, src++)
					{
			...
			else
			{
				...
				for (var y = 1; y < heightM1; y++)
				{
					for (var x = 1; x < widthM1; x++, motion++, src += pixelSize)
					{
						if (4 * *motion - motion[-width] - motion[width] - motion[1] - motion[-1] != 0)
						{
							src[RGB.R] = fillR;

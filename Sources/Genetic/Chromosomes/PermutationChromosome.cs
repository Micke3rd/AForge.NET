...
namespace AForge.Genetic
{
	...
	public class PermutationChromosome: ShortArrayChromosome
	{
		...
		public override void Generate()
		{
			...
			{
				ushort t;
				var j1 = rand.Next(length);
				var j2 = rand.Next(length);
		...
		{
			var p = (PermutationChromosome)pair;

			// check for correct pair
			if ((p != null) && (p.length == length))
			{
				var child1 = new ushort[length];
				var child2 = new ushort[length];
		...
		{
			var indexDictionary1 = CreateIndexDictionary(parent1);
			var indexDictionary2 = CreateIndexDictionary(parent2);

			// temporary array to specify if certain gene already
			// present in the child
			var geneIsBusy = new bool[length];
		...
		{
			var indexDictionary = new ushort[genes.Length];

...
namespace AForge.Genetic
{
	...
	///		{
	///			return ( System.Math.Cos( y ) * x * y ) / ( 2 - System.Math.Sin( x ) );
	///		}
	/// }
	/// ...
	...
	public abstract class OptimizationFunction2D : IFitnessFunction
	{
		...
		public double Evaluate(IChromosome chromosome)
		{
			...
			// get function value
			var functionValue = OptimizationFunction(xy[0], xy[1]);
		...
		public double[] Translate(IChromosome chromosome)
		{
			...
			var val = ((BinaryChromosome)chromosome).Value;
			// chromosome's length
			var length = ((BinaryChromosome)chromosome).Length;
			// length of X component
			var xLength = length / 2;
			// length of Y component
			var yLength = length - xLength;
			// X maximum value - equal to X mask
			var xMax = 0xFFFFFFFFFFFFFFFF >> (64 - xLength);
			// Y maximum value
			var yMax = 0xFFFFFFFFFFFFFFFF >> (64 - yLength);
			// X component

...
namespace AForge.Imaging.Formats
{
	...
	public class FITSCodec: IImageDecoder
	{
		...
		{
			var imageInfo = ReadHeader(stream);
		...
		public Bitmap DecodeFrame(int frameIndex,out ImageInfo imageInfo)
		{
			...
			// read required frame
			var image = ReadImageFrame(stream,this.imageInfo);
		...
		{
			var headerRecord = new byte[80];
			var recordsRead = 1;
			var endKeyWasFound = false;

			var imageInfo = new FITSImageInfo();
			...
			while (true)
			{
				...
				// get keyword
				var keyword = Encoding.ASCII.GetString(headerRecord,0,8);
				...
				else
				{
					...
					var strValue = Encoding.ASCII.GetString(headerRecord,10,70);

					// check important keywords
					if (keyword == "BITPIX  ")
					{
						var value = ExtractIntegerValue(strValue);
			...
			var dataPos = stream.Seek(0,SeekOrigin.Current);

			// data size
			var lineLength = imageInfo.Width * (Math.Abs(imageInfo.OriginalBitsPerPixl) / 8);
			var totalLines = imageInfo.Height * imageInfo.TotalFrames;
			var originalBitsPerPixel = imageInfo.OriginalBitsPerPixl;
		...
		private static unsafe Bitmap ReadImageFrame(Stream stream,FITSImageInfo imageInfo)
		{
			...
			// create new bitmap
			var image = (imageInfo.BitsPerPixel == 8) ?

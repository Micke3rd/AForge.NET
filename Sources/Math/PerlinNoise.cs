...
namespace AForge.Math
{
    ...
    public class PerlinNoise
    {
        ...
        private double SmoothedNoise(double x, double y)
        {
            ...
            var x0y0 = Noise(xInt, yInt);
            var x1y0 = Noise(xInt+1, yInt);
            var x0y1 = Noise(xInt, yInt+1);
            var x1y1 = Noise(xInt+1, yInt+1);

            // x interpolation
            var v1 = CosineInterpolate(x0y0, x1y0, xFrac);
            var v2 = CosineInterpolate(x0y1, x1y1, xFrac);
            // y interpolation

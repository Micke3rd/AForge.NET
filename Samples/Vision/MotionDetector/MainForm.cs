...
namespace MotionDetectorSample
{
	public partial class MainForm : Form
	{
		...
		private void openToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			{
				// create video source
				var fileSource = new AVIFileVideoSource(openFileDialog.FileName);
		...
		private void localVideoCaptureDeviceToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			{
				// create video source
				var videoSource = new VideoCaptureDevice(form.VideoDevice);
		...
		private void CloseVideoSource()
		{
			...
			for (var i = 0; (i < 50) && (videoSourcePlayer.IsRunning); i++)
			{
				Thread.Sleep(100);
			}
		...
		private void videoSourcePlayer_NewFrame(object sender, ref Bitmap image)
		{
			lock (this)
			{
				if (detector != null)
				{
					var motionLevel = detector.ProcessFrame(image);

					if (motionLevel > motionAlarmLevel)
					{
						// flash for 2 seconds
						flash = 2 * (1000 / alarmTimer.Interval);
					}

					// check objects' count
					if (detector.MotionProcessingAlgorithm is BlobCountingObjectsProcessing)
					{
						var countingDetector = (BlobCountingObjectsProcessing)detector.MotionProcessingAlgorithm;
		...
		private void DrawMotionHistory(Bitmap image)
		{
			...
			var t1 = (int)(motionAlarmLevel * 500);
			var t2 = (int)(0.075 * 500);

			for (int i = 1, n = motionHistory.Count; i <= n; i++)
			{
				var motionBarLength = (int)(motionHistory[n - i] * 500);
		...
		private void motionToolStripMenuItem_DropDownOpening(object sender, EventArgs e)
		{
			...
			for (var i = 0; i < motionDetectionItems.Length; i++)
			{
				motionDetectionItems[i].Checked = (i == motionDetectionType);
			}
			for (var i = 0; i < motionProcessingItems.Length; i++)
			{
			...
			var enabled = (motionDetectionType != 1);
		...
		private void defineMotionregionsToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (videoSourcePlayer.VideoSource != null)
			{
				...
				{
					var form = new MotionRegionsForm
					{
						VideoFrame = currentVideoFrame,
						MotionRectangles = detector.MotionZones
					};

					// show the dialog
					if (form.ShowDialog(this) == DialogResult.OK)
					{
						var rects = form.MotionRectangles;

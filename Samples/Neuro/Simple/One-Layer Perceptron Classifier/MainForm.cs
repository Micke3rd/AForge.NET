...
namespace Classifier
{
	...
	public class MainForm : System.Windows.Forms.Form
	{
		...
		{
			var dataSeries = new double[classesCount][,];
			var indexes = new int[classesCount];
		...
		void SearchSolution()
		{
			...
			// create perceptron
			var network = new ActivationNetwork(new ThresholdFunction(), 2, classesCount);
			...
			try
			{
				...
				while (!needToStop)
				{
					...
					{
						var k = (layer.Neurons[j].Weights[1] != 0) ? (-layer.Neurons[j].Weights[0] / layer.Neurons[j].Weights[1]) : 0;
						var b = (layer.Neurons[j].Weights[1] != 0) ? (-((ActivationNeuron)layer.Neurons[j]).Threshold / layer.Neurons[j].Weights[1]) : 0;
				...
				for (var i = 0; i < classesCount; i++)
				{
					...
					// weight 0
					var item = AddListItem(weightsList, neuronName);

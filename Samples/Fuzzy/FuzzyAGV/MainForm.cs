...
namespace FuzzyAGV
{
    public class MainForm : System.Windows.Forms.Form
    {
        ...
        void InitFuzzyEngine()
        {
            ...
            var fsNear = new FuzzySet("Near", new TrapezoidalFunction(15, 50, TrapezoidalFunction.EdgeType.Right));
            var fsMedium = new FuzzySet("Medium", new TrapezoidalFunction(15, 50, 60, 100));
            var fsFar = new FuzzySet("Far", new TrapezoidalFunction(60, 100, TrapezoidalFunction.EdgeType.Left));

            // Right Distance (Input)
            var lvRight = new LinguisticVariable("RightDistance", 0, 120);
            lvRight.AddLabel(fsNear);
            lvRight.AddLabel(fsMedium);
            lvRight.AddLabel(fsFar);

            // Left Distance (Input)
            var lvLeft = new LinguisticVariable("LeftDistance", 0, 120);
            lvLeft.AddLabel(fsNear);
            lvLeft.AddLabel(fsMedium);
            lvLeft.AddLabel(fsFar);

            // Front Distance (Input)
            var lvFront = new LinguisticVariable("FrontalDistance", 0, 120);
            lvFront.AddLabel(fsNear);
            lvFront.AddLabel(fsMedium);
            lvFront.AddLabel(fsFar);

            // Linguistic labels (fuzzy sets) that compose the angle
            var fsVN = new FuzzySet("VeryNegative", new TrapezoidalFunction(-40, -35, TrapezoidalFunction.EdgeType.Right));
            var fsN = new FuzzySet("Negative", new TrapezoidalFunction(-40, -35, -25, -20));
            var fsLN = new FuzzySet("LittleNegative", new TrapezoidalFunction(-25, -20, -10, -5));
            var fsZero = new FuzzySet("Zero", new TrapezoidalFunction(-10, 5, 5, 10));
            var fsLP = new FuzzySet("LittlePositive", new TrapezoidalFunction(5, 10, 20, 25));
            var fsP = new FuzzySet("Positive", new TrapezoidalFunction(20, 25, 35, 40));
            var fsVP = new FuzzySet("VeryPositive", new TrapezoidalFunction(35, 40, TrapezoidalFunction.EdgeType.Left));

            // Angle
            var lvAngle = new LinguisticVariable("Angle", -50, 50);
        ...
        {
            var g = Graphics.FromImage(pbTerrain.Image);
        ...
        private void GetMeasures()
        {
            ...
            pbTerrain.Image=CopyImage(OriginalMap);
            var b = pbTerrain.Image as Bitmap;
            var pPos = new Point(pbRobot.Left-pbTerrain.Left+5, pbRobot.Top-pbTerrain.Top+5);
            ...
            if ((b.GetPixel(pPos.X, pPos.Y).R==0)&&(b.GetPixel(pPos.X, pPos.Y).G==0)&&(b.GetPixel(pPos.X, pPos.Y).B==0))
            {
                ...
                var Msg = "The vehicle is on the solid area!";
                MessageBox.Show(Msg, "Error!");
                throw new Exception(Msg);
            }

            // Getting distances
            var pFrontObstacle = GetObstacle(pPos, b, -1, 0);
            var pLeftObstacle = GetObstacle(pPos, b, 1, 90);
            var pRightObstacle = GetObstacle(pPos, b, 1, -90);
        ...
        private Point GetObstacle(Point Start, Bitmap Map, int Inc, int AngleOffset)
        {
            ...
            var rad = ((Angle+90+AngleOffset)*System.Math.PI)/180;
            var IncX = 0;
            var IncY = 0;
            var Offset = 0;
        ...
        {
            var rad = ((Angle+90)*System.Math.PI)/180;
            var Offset = 0;
            var Inc = -4;

            Offset+=Inc;
            var IncX = Convert.ToInt32(Offset*System.Math.Cos(rad));
            var IncY = Convert.ToInt32(Offset*System.Math.Sin(rad));

            // Leaving the track 
            if (cbTrajeto.Checked)
            {
                var g = Graphics.FromImage(OriginalMap);
                var p1 = new Point(pbRobot.Left-pbTerrain.Left+pbRobot.Width/2, pbRobot.Top-pbTerrain.Top+pbRobot.Height/2);
                var p2 = new Point(p1.X+IncX, p1.Y+IncY);
        ...
        private void MoveCycle()
        {
            try
            {
                ...
                {
                    var mi = new MethodInvoker(AGVStep);
                    this.BeginInvoke(mi);

...
namespace Animat
{
    public partial class MainForm : Form
    {
        ...
        private void QLearningThread()
        {
            ...
            // exploration policy
            var tabuPolicy = (TabuSearchExploration)qLearning.ExplorationPolicy;
            ...
            while ((!needToStop)&&(iteration<learningIterations))
            {
                ...
                while ((!needToStop)&&((agentCurrentX!=agentStopX)||(agentCurrentY!=agentStopY)))
                {
                    ...
                    var currentState = GetStateNumber(agentCurrentX, agentCurrentY);
                    // get the action for this state
                    var action = qLearning.GetAction(currentState);
                    // update agent's current position and get his reward
                    var reward = UpdateAgentPosition(ref agentCurrentX, ref agentCurrentY, action);
                    // get agent's next state
                    var nextState = GetStateNumber(agentCurrentX, agentCurrentY);
        ...
        private void SarsaThread()
        {
            ...
            int agentCurrentX, agentCurrentY;
            // exploration policy
            var tabuPolicy = (TabuSearchExploration)sarsa.ExplorationPolicy;
            var explorationPolicy = (EpsilonGreedyExploration)tabuPolicy.BasePolicy;
            ...
            while ((!needToStop)&&(iteration<learningIterations))
            {
                ...
                var previousState = GetStateNumber(agentCurrentX, agentCurrentY);
                var previousAction = sarsa.GetAction(previousState);
                // update agent's current position and get his reward
                var reward = UpdateAgentPosition(ref agentCurrentX, ref agentCurrentY, previousAction);
                ...
                while ((!needToStop)&&((agentCurrentX!=agentStopX)||(agentCurrentY!=agentStopY)))
                {
                    ...
                    var nextAction = sarsa.GetAction(nextState);
                    // do learning of the agent - update his Q-function
        ...
        private void ShowSolutionThread()
        {
            ...
            while (!needToStop)
            {
                ...
                var currentState = GetStateNumber(agentCurrentX, agentCurrentY);
                // get the action for this state
                var action = (qLearning!=null) ? qLearning.GetAction(currentState) : sarsa.GetAction(currentState);
                // update agent's current position and get his reward
                var reward = UpdateAgentPosition(ref agentCurrentX, ref agentCurrentY, action);
        ...
        {
            var c1 = (map[y-1, x-1]!=0) ? 1 : 0;
            var c2 = (map[y-1, x]!=0) ? 1 : 0;
            var c3 = (map[y-1, x+1]!=0) ? 1 : 0;
            var c4 = (map[y, x+1]!=0) ? 1 : 0;
            var c5 = (map[y+1, x+1]!=0) ? 1 : 0;
            var c6 = (map[y+1, x]!=0) ? 1 : 0;
            var c7 = (map[y+1, x-1]!=0) ? 1 : 0;
            var c8 = (map[y, x-1]!=0) ? 1 : 0;

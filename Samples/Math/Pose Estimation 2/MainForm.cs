...
namespace PoseEstimation
{
	public partial class MainForm: Form
	{
		...
		private void openSampleToolStripMenuItem_Click(object sender,EventArgs e)
		{
			...
			var sampleIndex = int.Parse((string)menuItem.Tag);
		...
		private void OpenEmbeddedImage(string imageName)
		{
			...
			var image = new Bitmap(assembly.GetManifestResourceStream("PoseEstimation.Samples." + imageName));
			OpenImage(image);
		}
		...
		private void pictureBox_Paint(object sender,PaintEventArgs e)
		{
			...
			{
				var cx = imageSize.Width / 2;
				var cy = imageSize.Height / 2;
				...
				{
					var projectedAxes = PerformProjection(axesModel,
		...
		private AForge.Point[] PerformProjection(Vector3[] model,Matrix4x4 transformationMatrix,int viewSize)
		{
			...
			{
				var scenePoint = (transformationMatrix * model[i].ToVector4()).ToVector3();
		...
		{
			var tag = int.Parse((string)textBox.Tag);
			var pointIndex = tag / 10;
			var coordinateIndex = tag % 10;
			...
			{
				var pointLabel = (Label)modelPointsGroupBox.Controls[string.Format("modelPoint{0}Label",pointIndex + 1)];
		...
		private void EstimatePose()
		{
			try
			{
				...
				{
					var distanceToCenter = (modelPoint - modelCenter).Norm;
				...
				if (!useCoplanarPosit)
				{
					var posit = new Posit(modelPoints,focalLength);
				...
				{
					var coposit = new CoplanarPosit(modelPoints,focalLength);
